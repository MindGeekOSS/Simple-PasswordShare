...
class SimpleOTP:
    """
    A simple one-time password retreival web based application.
    """
    ...
    def _renderView(self, template, params):
        ...
        Returns:
          The final html content to be displayed (string)
        """
    ...
    def index(self, **args):
        ...
        Returns:
          The corresponding HTML template to be rendered to the user (string)
        """
        self._sendHeaders()
    ...
    def getPassword(self, **args):
        ...
        # Attempt to connect, create the table if it doesn't exist, then get the password
        try:
            con = sql.connect(cherrypy.config.get('db.name'))
            cur = con.cursor()
            cur.execute("CREATE TABLE IF NOT EXISTS otp (hash TEXT, password TEXT, date_created INT)")
...
import os.path
# Load the app config
conf = os.path.join(os.path.dirname(__file__), 'data/simpleotp.conf')


# Run the SimpleOTP class as a CherryPy web app as it's being called directly from command line
